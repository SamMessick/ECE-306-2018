...
uint16_t ir_black_reading;
uint16_t ir_white_reading;
uint8_t calibrated;
...
void updateDisplay(void){ 
  if(calibrated && ir_ready_to_print)
  {
    word4 = "          ";
    if(circle_time*2 >= 300)
      waiting = false;
    hex_to_dec(word4, (uint16_t) (circle_time*2));
    word4[COLUMN5] = word4[COLUMN4];
    word4[COLUMN4] = '.';
    LCD_print("Passing thr", "   Loop   ", "          ", word4);
    print_detector_values();
  }
  else if(ir_ready_to_print)
  {
    print_detector_values();
