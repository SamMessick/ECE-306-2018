...
uint16_t counter_A00;               // 1 msec counting interval for Timer A1.0
...
volatile uint16_t circle_time;
volatile uint8_t time_for_menu_update_lol;  // allows for screen refresh

void Init_Timer_A0(void) {
  // Initialize Timer A1 and activate display update
  TA0CTL = TASSEL__ACLK;
  TA0CTL |= (TACLR | MC_1 | ID_1);       // SMCLK Clock source on up mode at 1 MHz
  TA0CTL |= TAIDEX__1;                   
  TA0CCR0 = TWO_MSEC;                        // interrupt time set to 1/1000 second
...
__interrupt void Timer0_A0_ISR(void){
  ...
  if(counter_A00)            // Quarter-second elapsed
  {
    time_for_menu_update_lol = true; //memes.jpg
